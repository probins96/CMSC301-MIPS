...
int RegisterTable::getNum(string registerName) {
  for (int i = 0; i < 2 * NumRegisters; i++)
    if ( myRegisters[i].name == registerName)
...
string RegisterTable::getName(int registerNumber) {
  for (int i = 0; i < 2 * NumRegisters; i++)
    if (myRegisters[i].number == registerNumber)
